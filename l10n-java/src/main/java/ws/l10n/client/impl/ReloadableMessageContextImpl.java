...
import ws.l10n.client.Options;
import ws.l10n.client.ReloadableMessageContext;
...
public class ReloadableMessageContextImpl implements ReloadableMessageContext {
    ...
    public String getMessage(String key, String defaultMessage, Locale locale) {
        ...
        String message = messageBundle != null ? messageBundle.getMessage(key) : null;
        if (message == null) {
            if (defaultMessage != null) {
                return defaultMessage;
            }
            if (options.isShowKeyIfNotExist()) {
                return key;
            } else {
                throw new IllegalArgumentException("No such message found with key '"
                        + key + "' and locale " + locale.getDisplayName());
            }
        }
        return message;
    }
